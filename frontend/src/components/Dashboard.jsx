...
import { Code2, Brain, FileText, BookOpen, MessageSquare, Map, Mic, TrendingUp, Calendar, Award, Building2 } from 'lucide-react';

export default function Dashboard() {
  const { user } = useAuth();
  const [displayUser, setDisplayUser] = useState(user);

  // Listen for real-time updates from Aptitude tests
  useEffect(() => {
    setDisplayUser(user);
    
    const handleUserUpdate = (e) => {
      setDisplayUser(prev => ({ ...prev, ...e.detail }));
    };
  ...
  const features = [
    ...
    { title: 'Companies', desc: 'Track top companies hiring right now.', icon: Building2, color: 'bg-indigo-500', link: '/companies' },
  ];
    ...
    <div className="space-y-8">
      ...
      <div className="relative overflow-hidden bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 rounded-3xl p-8 text-white shadow-2xl">
        ...
        <div className="relative z-10">
          <h1 className="text-3xl md:text-4xl font-bold mb-3">Welcome back, {displayUser?.name || 'Student'}! 👋</h1>
          ...
          <div className="flex flex-wrap gap-4 mt-6">
            <div className="bg-white/20 backdrop-blur-sm px-4 py-2 rounded-xl flex items-center gap-2">
              ...
              <span className="font-semibold">Level {displayUser?.level || 1} {getLevelTitle(displayUser?.level || 1)}</span>
            </div>
            <div className="bg-white/20 backdrop-blur-sm px-4 py-2 rounded-xl flex items-center gap-2">
              <Calendar className="w-5 h-5 text-green-300" />
              <span className="font-semibold">{displayUser?.streak?.current || 0} Day Streak 🔥</span>
            </div>
            <div className="bg-white/20 backdrop-blur-sm px-4 py-2 rounded-xl flex items-center gap-2">
              <TrendingUp className="w-5 h-5 text-blue-300" />
              <span className="font-semibold">{displayUser?.xp || 0} XP</span>
            </div>
          </div>
          ...
          <div className="mt-6">
            <div className="flex justify-between text-sm mb-2">
              <span className="text-blue-100">Progress to Level {(displayUser?.level || 1) + 1}</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
          ...
          <p className="text-3xl font-bold text-gray-900">{displayUser?.progress?.problemsSolved || 0}</p>
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
          ...
          <p className="text-3xl font-bold text-gray-900">{displayUser?.progress?.aptitudeScore || 0}%</p>
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
          ...
          <p className="text-3xl font-bold text-gray-900">{displayUser?.progress?.testsTaken || 0}</p>
