...
export default function ResumeAnalyzer() {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const res = await axios.post('https://placement-portal-api.onrender.com/api/resume/analyze', formData, {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-8 rounded-t-2xl">
        ...
        <p className="opacity-90">Get instant feedback on your resume from our AI recruiter</p>
      </div>
        ...
        {analysis && (
          <div className="space-y-6">
            ...
            <div className="bg-yellow-50 p-4 rounded-xl">
              <h4 className="font-bold text-yellow-700 mb-2">💡 Suggestions</h4>
              <ul className="list-disc list-inside space-y-1">
            ...
            <div className="bg-purple-50 p-4 rounded-xl">
              <h4 className="font-bold text-purple-700 mb-2">🎯 Recommended Roles</h4>
              <div className="flex flex-wrap gap-2">
                {analysis.recommendedRoles?.map((r, i) => (
                  <span key={i} className="bg-purple-200 text-purple-800 px-3 py-1 rounded-full text-sm">{r}</span>
                ))}
              </div>
            </div>
          </div>
